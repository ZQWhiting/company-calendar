...
const { Employee, Calendar, Event, Party, Inbox } = require('../../models');
...
router.post(':id/group/:group_id', (req, res) => {
	Employee.findOne({
		where: {
			id: req.params.id,
		},
		...
		.then((employee) => {
			...
			Party.findOne({ where: { id: req.params.group_id } }).then(
				(party) => {
					if (!party) {
						res.status(404).json({
							message: 'No party found with this id',
						});
						return;
					}
					employee.addParty(party);
